...
const qrStore = {}; const agentesConfig = {}; // Armazena agentes por número

// Limites por plano const limitesPlano = { gratuito: { maxMensagens: 30, maxAgentes: 1 }, standard: { maxMensagens: 10000, maxAgentes: 1 }, ultra: { maxMensagens: Infinity, maxAgentes: 3 } };
...
app.post('/zapagent', async (req, res) => { const { nome, tipo, descricao, prompt, numero, plano = 'gratuito' } = req.body; if (!numero || !prompt) return res.status(400).json({ error: 'Número ou prompt ausente' });

const planoAtual = plano.toLowerCase(); const agenteDoNumero = agentesConfig[numero] || [];

if (!Array.isArray(agenteDoNumero)) agentesConfig[numero] = [];

if (agenteDoNumero.length >= limitesPlano[planoAtual].maxAgentes) { return res.status(403).json({ error: ⚠️ Limite de agentes (${limitesPlano[planoAtual].maxAgentes}) atingido para o plano ${planoAtual} }); }
...
if (qr) qrStore['351967578444'] = qr; // Apenas para teste fixo de número
...
for (let agente of agentes) {
  const { plano, mensagens, prompt } = agente;
  const limite = limitesPlano[plano]?.maxMensagens ?? 30;
  ...
  try {
    const resposta = await gerarRespostaIA(texto, prompt);
